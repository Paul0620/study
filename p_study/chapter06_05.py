...
# Futures 동시성
# 비동기 작업 실행
# 지연시간(Block) CPU 및 리소스 낭비 방지 -> (File)Network I/O 관련 작업일 때 -> 동시성 활용 권장
# 비동기 작업과 적합한 프로그램일 경우 압도적으로 성능 향상

# futures - 비동기 실행을 위한 API를 고수준으로 작성, 사용하기 쉽도록 개선
# concurrent.Futures
# 1. 멀티쓰레딩/멀티프로세싱 API 통일 -> 매우 사용하기 쉬워짐
...
# GIL(Global Interpreter Lock) - 두 개 이상의 스레드가 동시에 실행 될 때 하나의 자원을 엑세스하는 경우
# -> 문제점을 방지하기 위해 GIL이 실행된다. 리소스 전체에 락이 걸리게 된다. -> Context Switch(문맥 교환)
# GIL 우회방법 - 멀티프로세싱 사용, CPython을 이용
...
def main():
    ...
    with ThreadPoolExecutor() as executor:
        ...
        # # 실패
        # print(
        #     "\nPending ones after waiting for 2seconds : " + str(result.not_done) + "\n"
        # )
